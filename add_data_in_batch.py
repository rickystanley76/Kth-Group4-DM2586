...
def import_data(client: WeaviateClient, batch_size=200) -> None:
    mm_coll = client.collections.get(COLLECTION_NAME)

    # Read all image files
    image_files = list(imgdir.glob("*.jpg"))
    ...
    for i in range(0, len(image_files), batch_size):
        ...
        # Insert the batch of images
        insert_response = mm_coll.data.insert_many(data_objs)

        print(f"Batch {i // batch_size + 1} insertions complete.")
...
def main():
    ...
    import_data(client, batch_size=200)  # Specify the batch size here
    demo_query(client)
